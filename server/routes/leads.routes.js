...
router.post('/', authenticateToken, asyncHandler(async (req, res) => {
  ...
  const createdLead = await sb.insert('leads', {
    ...
    current_owner_id: automaticManager?.id || req.user.id,
    workflow_status: automaticManager ? 'assigned_to_manager' : 'new',
  ...
  await sb.insert('lead_activity', { lead_id: lead.id, event_type: 'lead_created', performed_by_user_id: req.user.id, previous_status: null, new_status: lead.workflow_status, metadata: { source: lead.source, manager_id: automaticManager?.id || null } });
  ...
  if (automaticManager) {
    await notifyUsers([automaticManager.id], { lead_id: lead.id, type: 'manager_assignment', title: 'New lead generated by employee', message: `${lead.lead_id} - ${clinicName}` });
  }
