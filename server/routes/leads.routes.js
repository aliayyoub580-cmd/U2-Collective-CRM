...
async function assignUniqueLeadId(databaseId) {
  const { data: existingLeads } = await sb.list('leads', {
    select: 'lead_id',
    likes: [['lead_id', 'LD-']],
    limit: 10000
  });
  const highestLeadNumber = (existingLeads || []).reduce((highest, row) => {
    const match = /^LD-(\d+)$/.exec(row.lead_id || '');
    return match ? Math.max(highest, Number(match[1])) : highest;
  }, 0);
  let nextNumber = Math.max(Number(databaseId), highestLeadNumber + 1);

  // A concurrent creation can choose the same next number. The unique index
  // remains the final authority, so advance and retry when that happens.
  for (let attempt = 0; attempt < 20; attempt += 1, nextNumber += 1) {
    try {
      return await sb.update('leads', [['id', 'eq', databaseId]], {
        lead_id: formatLeadId(nextNumber)
      });
    } catch (error) {
      if (!/duplicate key|unique constraint/i.test(error.message)) throw error;
    }
  }
  throw new Error('Unable to allocate a unique lead ID');
}
...
router.post('/', authenticateToken, asyncHandler(async (req, res) => {
  ...
  // A Manager queue is ownership-based. Automatically route employee-created
  // leads only when the destination is unambiguous; otherwise Admin assigns one.
  let automaticManager = null;
  let selectedCaller = null;
  if (req.user.employee_type === 'lead_generator') {
    const { data: activeManagers } = await sb.list('users', {
      select: 'id,name',
      filters: [['role', 'eq', 'Manager'], ['status', 'eq', 'active']],
      order: 'id.asc',
      limit: 2
    });
    if ((activeManagers || []).length === 1) automaticManager = activeManagers[0];
  } else if (req.user.role === 'CEO' && assigned_to) {
    selectedCaller = await sb.one('users', {
      select: 'id,name,employee_type,status',
      filters: [['id', 'eq', assigned_to]]
    });
    if (!selectedCaller || selectedCaller.status !== 'active' || selectedCaller.employee_type !== 'caller') {
      return res.status(400).json({ error: 'Select an active Caller' });
    }
  }
  ...
  const createdLead = await sb.insert('leads', {
    ...
    source,
    service_interested,
    status: selectedCaller ? 'Assigned' : 'New',
    assigned_to: selectedCaller?.id || null,
    created_by: req.user.id,
    lead_generator_id: req.user.employee_type === 'lead_generator' ? req.user.id : null,
    manager_id: automaticManager?.id || null,
    caller_id: selectedCaller?.id || null,
    current_owner_id: selectedCaller?.id || automaticManager?.id || req.user.id,
    workflow_status: selectedCaller ? 'assigned_to_caller' : automaticManager ? 'assigned_to_manager' : 'new',
    assigned_to_manager_at: automaticManager ? new Date().toISOString() : null,
    assigned_to_caller_at: selectedCaller ? new Date().toISOString() : null,
    assigned_to_caller_by: selectedCaller ? req.user.id : null,
    submitted_at: null,
    notes
  });
  const lead = await assignUniqueLeadId(createdLead.id);
  ...
  // Audit and notification failures must never turn a committed lead into an
  // apparent creation failure. This is especially important during staged
  // Supabase migrations where a workflow table may still have RLS enabled.
  const sideEffects = [
    logActivity(req.user.id, 'Created Lead: ' + clinicName, 'leads', lead.id),
    sb.insert('lead_activity', { lead_id: lead.id, event_type: 'lead_created', performed_by_user_id: req.user.id, previous_status: null, new_status: lead.workflow_status, metadata: { source: lead.source, manager_id: automaticManager?.id || null } }),
    notifyUsers([req.user.id], { lead_id: lead.id, type: 'lead_created', title: 'Lead Created Successfully', message: `${lead.lead_id} - ${clinicName}` })
  ];
  if (automaticManager) sideEffects.push(
    notifyUsers([automaticManager.id], { lead_id: lead.id, type: 'manager_assignment', title: 'New lead generated by employee', message: `${lead.lead_id} - ${clinicName}` })
  );
  if (selectedCaller) sideEffects.push(
    notifyUsers([selectedCaller.id], { lead_id: lead.id, type: 'lead_assigned', title: 'New Lead Assigned', message: `${lead.lead_id} - ${clinicName}` })
  );
