...
const blank = () => ({ call_status: '', contact_person_name: '', contact_person_role: '', call_date: new Date().toISOString().split('T')[0], call_time: '', call_duration: '', interest_status: '', follow_up_date: '', notes: '', additional_contact_information: '' });
...
export default function CallerTasksPage() {
  ...
  const save = async (event) => { event.preventDefault(); if (!form.call_status || !form.call_date || !form.interest_status) return setFormError('Call status, call date and interest status are required.'); if (form.interest_status === 'Needs Follow-Up' && !form.follow_up_date) return setFormError('Select when to follow up.'); setSaving(true); setFormError(''); try { await api.post(`/caller/leads/${selected.id}/complete`, { ...form, follow_up_required: form.interest_status === 'Needs Follow-Up' }); setSelected(null); setMessage('Caller outcome submitted to the Manager and Admin.'); await load(); } catch (error) { setFormError(error.response?.data?.error || 'Unable to submit caller outcome.'); } finally { setSaving(false); } };
  const columns = [
