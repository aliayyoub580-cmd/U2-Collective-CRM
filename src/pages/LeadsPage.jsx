...
import { Edit, Eye, Plus, RefreshCw, Search, Trash2, UserCheck, Users } from 'lucide-react';
...
export default function LeadsPage() {
  ...
  const canAssign = hasRole('CEO', 'Manager');
  ...
  useEffect(() => {
    if (!canAssign) return;
    api.get('/employees/users/all', { params: { employee_type: 'caller' } }).then((res) => setUsers(res.data.users || [])).catch(() => setUsers([]));
  }, [canAssign]);
  ...
  const columns = [
    ...
    { header: 'Actions', align: 'right', render: (r) => <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 4 }}><button onClick={() => navigate(`/leads/${r.id}`)} className="p-2 rounded-lg text-blue-600" title="View lead"><Eye size={15}/></button>{canAssign && <button onClick={() => openAssign(r)} className="p-2 rounded-lg text-green-600" title="Assign caller"><UserCheck size={15}/></button>}{canCreate && <button onClick={() => openEdit(r)} className="p-2 rounded-lg text-slate-600" title="Edit lead"><Edit size={15}/></button>}{hasRole('CEO') && <button onClick={() => setDeleting(r.id)} className="p-2 rounded-lg text-red-600" title="Delete lead"><Trash2 size={15}/></button>}</div> }
  ];
