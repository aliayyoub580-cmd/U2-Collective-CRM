...
import { Edit, Eye, Plus, RefreshCw, Search, Send, Trash2, Users } from 'lucide-react';
...
const emptyForm = { company_name: '', clinic_website: '', clinic_email: '', clinic_specialty: '', clinic_phone: '', state: '', city: '', source: '', source_other: '', practice_size: '', assigned_to: '', notes: '' };
...
export default function LeadsPage() {
  ...
  const [assigningLead, setAssigningLead] = useState(false); const [assignError, setAssignError] = useState('');
  const [managerLead, setManagerLead] = useState(null); const [managerForm, setManagerForm] = useState({ manager_id: '', caller_id: '', priority: 'Normal', due_date: '', instructions: '' });
  ...
  const assignManager = async (event) => { event.preventDefault(); if (!managerForm.manager_id && !managerForm.caller_id) return setAssignError('Select a Manager, a Caller, or both.'); setAssigningLead(true); setAssignError(''); try { if (managerForm.manager_id && String(managerForm.manager_id) !== String(managerLead.manager_id || '')) await api.post(`/admin/leads/${managerLead.id}/assign-manager`, managerForm); if (managerForm.caller_id && String(managerForm.caller_id) !== String(managerLead.caller_id || managerLead.assigned_to || '')) await api.patch(`/leads/${managerLead.id}/assign`, { assigned_to: managerForm.caller_id }); setManagerLead(null); setNotice('Lead assignments updated successfully.'); load(); } catch (e) { setAssignError(e.response?.data?.error || 'Unable to update lead assignments.'); } finally { setAssigningLead(false); } };
  ...
  const columns = [
    ...
    { header: 'Website', render: (r) => r.clinic_website ? <a href={websiteHref(r.clinic_website)} target="_blank" rel="noreferrer" style={{ color: '#2563EB', textDecoration: 'underline' }} title={r.clinic_website}>Open website</a> : <span style={{ color: '#94A3B8' }}>—</span> },
  ...
  return <div><PageHeader title={user?.role === 'Manager' ? 'Manager Review Queue' : 'Healthcare Leads'} subtitle={`${total} leads`} actions={<><button onClick={load} className="p-2 rounded-xl text-slate-600"><RefreshCw size={16}/></button>{canCreate && <PrimaryButton onClick={openCreate} icon={Plus}>Add Lead</PrimaryButton>}</>} />
    ...
    <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', padding: 16, background: '#FFF', border: '1px solid #E2E8F0', borderRadius: 16, marginBottom: 18 }}><div className="relative" style={{ flex: 1, minWidth: 220 }}><Search size={15} className="absolute left-3 top-3 text-slate-400"/><input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} placeholder="Search ID, clinic, website, email, phone, state or source" style={{ width: '100%', height: 40, paddingLeft: 36, border: '1px solid #E2E8F0', borderRadius: 10 }}/></div><select value={status} onChange={(e) => setStatus(e.target.value)}><option value="">All statuses</option>{STATUSES.map(x => <option key={x}>{x}</option>)}</select><select value={state} onChange={(e) => setState(e.target.value)}><option value="">All states</option>{STATES.map(x => <option key={x}>{x}</option>)}</select><select value={source} onChange={(e) => setSource(e.target.value)}><option value="">All sources</option>{SOURCES.map(x => <option key={x}>{x}</option>)}</select></div>
