...
import { Eye, Mail, RefreshCw, Search, Send, UserCheck, Users } from 'lucide-react';
...
const blank = { assignee: '', priority: 'Normal', due_date: '', instructions: '', review_notes: '', email_stage: 'initial', recipient_email: '', cc: '', subject: '', message_body: '', interest_status: 'interested', manager_notes: '', communication_summary: '', closure_reason: '', decision_maker_name: '', decision_maker_role: '', preferred_contact_method: '', next_step: '', next_follow_up_at: '', reason_not_interested: '', conversion_details: '', confirmed: false };
...
export default function ManagerLeadsPage() {
  ...
  const open = async (kind, lead) => {
    ...
    if (kind === 'generator' || kind === 'caller') {
      const type = kind === 'generator' ? 'lead_generator' : 'caller';
      try { const response = await api.get('/employees/users/all', { params: { employee_type: type } }); setPeople(response.data.users || []); } catch { setPeople([]); }
    }
    if (kind === 'detail') {
      setDetail(null);
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      if (action === 'generator') await api.post(`/manager/leads/${id}/assign-lead-generator`, { lead_generator_id: form.assignee, priority: form.priority, due_date: form.due_date || null, instructions: form.instructions });
      ...
      if (action === 'email') await api.post(`/manager/leads/${id}/email`, { email_stage: form.email_stage, recipient_email: form.recipient_email, cc: form.cc, subject: form.subject, message_body: form.message_body });
  ...
  const actions = (lead) => {
    ...
    return <div className="flex justify-end gap-1 flex-wrap">
      ...
      {['assigned_to_manager','new'].includes(workflow) && <button className="p-2 text-violet-600" title="Assign Lead Generator" onClick={() => open('generator', lead)}><Users size={15}/></button>}
      ...
      {['approved_for_caller','caller_completed','manager_follow_up'].includes(workflow) && <button className="p-2 text-emerald-600" title="Assign Caller" onClick={() => open('caller', lead)}><UserCheck size={15}/></button>}
  ...
  return <div>
    ...
    <Modal isOpen={!!action && action !== 'detail'} onClose={() => setAction(null)} title={pretty(action)} size="lg"><form onSubmit={submit}>
      ...
      {(action === 'generator' || action === 'caller') && <div className="grid gap-4 md:grid-cols-2"><FormSelect label={action === 'generator' ? 'Lead Generator' : 'Caller'} required value={form.assignee} onChange={(e) => setForm({...form,assignee:e.target.value})}><option value="">Select employee</option>{people.map((person) => <option key={person.id} value={person.id}>{person.name} ({person.email})</option>)}</FormSelect><FormSelect label="Priority" value={form.priority} onChange={(e) => setForm({...form,priority:e.target.value})}>{['Low','Normal','High','Urgent'].map((item) => <option key={item}>{item}</option>)}</FormSelect><FormInput label="Due Date" type="date" value={form.due_date} onChange={(e) => setForm({...form,due_date:e.target.value})}/><div className="md:col-span-2"><FormTextarea label="Instructions" rows={4} value={form.instructions} onChange={(e) => setForm({...form,instructions:e.target.value})}/></div></div>}
